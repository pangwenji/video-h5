...
declare namespace JessibucaProMulti {
    ...
    interface Config {
        ...
        /**
         * 是否显示ptz 的左上，右上，左下，右下按钮
         */
        ptzMoreArrowShow?: boolean,
        ...
        /**
         * 是否显示ptz aperture（聚焦） 按钮
         */
        ptzApertureShow?: boolean,

        /**
         * 是否显示ptz focus（光圈） 按钮
         */
        ptzFocusShow?: boolean,
        ...
        /**
         * decoder 是否使用cdn地址（绝对地址）
         */
        isDecoderUseCDN?: boolean,
        ...
        /**
         * 是否支持hls265(使用新的hls265解码器)
         */
        supportHls265?: boolean,


        /**
         * 是否需要抛出SEI数据（如果流数据里面有）
         */
        isEmitSEI?: boolean,

        /**
         * 调用pause()方法，再次调用play()方法的时候使用最后一帧来显示。
         */
        pauseAndNextPlayUseLastFrameShow?: boolean,
        /**
         * 硬解码（MSE,WCS）是否在worker里面请求数据+解封装
         */
         ...
         * wcs使用webgl2渲染画面
         */
        wcsUseWebgl2Render?: boolean,


        /**
         * 是否SRS服务器的webrtc，不推荐
         */
        isWebrtcForSRS?: boolean,


        /**
         * 是否其他服务器的webrtc
         */
        isWebrtcForOthers?: boolean,
...
declare class JessibucaProMulti {
     ...
     * @param windowIndex
     */
    getWindowItem(windowIndex?: number): {
        index: number,
        $player: HTMLElement,
    ...
    getVolume(windowIndex?: number): number;
     ...
     * 返回视频信息
     */
    getVideoInfo(windowIndex?: number): object;

    /**
     * 返回音频信息
     */
    getAudioInfo(windowIndex?: number): object;
     ...
     * @param config
     */
    updateWatermark(config: object | [], windowIndex?: number): void;

    /**
     * 移除水印
     */
    removeWatermark(windowIndex?: number): void;

    /**
     * 获取视频信息
     */
    getVideoInfo(windowIndex?: number): { width: number, height: number, encType: string }


    /**
     * 获取音频信息
     */
    getAudioInfo(windowIndex?: number): { sampleRate: number, channels: number, encType: string }
     ...
     * @param text
     */
    updateLoadingText(text: string, windowIndex?: number): void;

    /**
     *
     * @param flag
     */
    updateIsEmitSEI(flag: boolean, windowIndex?: number): void;

    /**
     *
     * @param aspectRatio
     */
    setAspectRatio(aspectRatio: string, windowIndex?: number): void;

    /**
     * 获取flv流的metadata信息
     */
    getFlvMetaData(windowIndex?: number): object;


    /**
     * 支持获取`ptz` gb28181指令封装
     * @param cmd
     * @param speed
     */
    getPTZCmd(cmd: string, speed: number, windowIndex?: number): string;

    /**
     * 获取到自定义的`按钮DOM Element`，可以执行业务上面的操作。
     */
    getExtendBtnList(windowIndex?: number): Array<{
        name: string,
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.selected, callback: (windowIndex: number) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.mouseOut, callback: (windowIndex: number) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.mouseOver, callback: (windowIndex: number) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.mouseUp, callback: (windowIndex: number) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.dragenter, callback: (windowIndex: number, event: any) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.dragleave, callback: (windowIndex: number, event: any) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.MULTI_EVENTS.drop, callback: (windowIndex: number, event: any) => void): void;
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.ERROR.decoderWorkerInitError, callback: (windowIndex: number, error: string) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.ERROR.videoElementPlayingFailed, callback: (windowIndex: number, error: string) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.streamEnd, callback: () => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.ERROR.videoElementPlayingFailedForWebrtc, callback: (windowIndex: number, error: string) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.flvMetaData, callback: (windowIndex: number, data: object) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.videoSEI, callback: (windowIndex: number, data: {
        ts: string,
        data: Uint8Array
    }) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.webrtcOnConnectionStateChange, callback: (windowIndex: number, data: object) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.webrtcOnIceConnectionStateChange, callback: (windowIndex: number, data: object) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.aiObjectDetectorInfo, callback: (windowIndex: number, data: object) => void): void
     ...
     * @param callback
     */
    on(event: JessibucaProMulti.EVENTS.networkDelayTimeout, callback: (windowIndex: number) => void): void
    /**
     * 监听方法
     *
     @example
